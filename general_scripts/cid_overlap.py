...
t3 = pd.read_csv("inputs/CELL-LINES.csv")

s1 = set(t1["Pubchem CID"].dropna().astype(float).astype(int))
s2 = set(t2["Pubchem CID"].dropna().astype(float).astype(int))
s3 = set(t3["Pubchem CID"].dropna().astype(float).astype(int))

common_cids = s1 & s2 & s3

df_out = pd.DataFrame(sorted(common_cids), columns=["cid"])
df_out.to_csv("outputs/cid_overlap.csv", index=False)
