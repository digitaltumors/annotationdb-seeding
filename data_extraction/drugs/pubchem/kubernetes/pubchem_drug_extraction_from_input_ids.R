...
out_dir <- args$out_dir %||% "output_data/union/complete"
...
allowed_aids_path <- if (file.exists("homosapien_aids.txt")) "homosapien_aids.txt" else file.path(dirname(in_csv), "homosapien_aids.txt")
...
# ---------------------------
inchikey_to_cid <- function(key, max_retries = 6) {
    ...
    for (i in seq_len(max_retries)) {
        ...
        if (!identical(res, "RETRY")) {
            return(as.character(res %||% character(0)))
        }
        wait_time <- min(10, 1.5 * (2^(i - 1)))
        Sys.sleep(wait_time + runif(1, 0, 1))
    }
...
if (any(needs_res) && !is.null(inchikey_col)) {
    ...
    if (length(keys_to_res) > 0) {
        # Random initial stagger to spread out pods starting simultaneously
...
if (file.exists(master_union_path)) {
    ...
    gc()
} else if (file.exists(union_out_path)) {
    ...
    printf("[%s] Found %d existing CIDs in local file.", ts(), length(already_done_cids))
...
# ---------------------------
if (length(props_todo) > 0) {
    printf("[%s] Synonym fetch: processing %d CID(s) from this run", ts(), length(props_todo))

    # Batch them for efficiency
    syn_mini <- split(props_todo, ceiling(seq_along(props_todo) / 10L))
...
bio_all <- data.table()
if (length(props_todo) > 0) {
    printf("[%s] Bioassays: processing %d CID(s)", ts(), length(props_todo))
    pbar <- match.call()$pbar %||% FALSE

    bio_chunks <- split(seq_along(props_todo), ceiling(seq_along(props_todo) / 5L))

    local_bio <- list()
    bi <- 0L

    for (idx in bio_chunks) {
        bi <- bi + 1L
        ids <- props_todo[idx]

        printf(
            "[%s]   Bioassays mini-batch %d/%d | CIDs: %d",
            ts(), bi, length(bio_chunks), length(ids)
        )
...
# ---------------------------
# Final union_out.csv write (batch-isolated, no global merge)
# ---------------------------
...
if (!is.null(props_dt_all) && NROW(props_dt_all) > 0) {
    ...
    # Merge mapped_name from input IDs
    inp_summary <- inp[, .(added_names = paste(input_id, collapse = "; ")), by = cid]
    inp_summary[, cid := canon_cid(cid)]

    if ("mapped_name" %in% names(props_dt_all)) {
        props_dt_all <- merge(props_dt_all, inp_summary, by = "cid", all.x = TRUE, suffixes = c("", ".new"))
        props_dt_all[, mapped_name := vapply(seq_len(.N), function(i) {
            merge_mapped_name(mapped_name[i], added_names[i])
        }, character(1))]
        props_dt_all[, added_names := NULL]
    } else {
        props_dt_all <- merge(props_dt_all, inp_summary, by = "cid", all.x = TRUE)
        setnames(props_dt_all, "added_names", "mapped_name")
    }

    # Write to batch-local file (no global overwrite)
    append_fwrite_aligned(props_dt_all, union_out_path, "Properties")
    printf("[%s] Wrote %d rows to %s", ts(), nrow(props_dt_all), union_out_path)
}

# Final cleanup
if (exists("props_dt_all")) rm(props_dt_all)
gc()
