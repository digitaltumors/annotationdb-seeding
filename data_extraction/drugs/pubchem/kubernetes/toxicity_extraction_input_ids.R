...
get_batch_cids_from_input_ids <- function(props_dt, batch_input_ids_csv) {
    ...
    u_expanded <- props_dt[, .(input_id = unlist(strsplit(as.character(mapped_name), ";\\s*"))), by = cid]
...
get_tox <- function(pubchem_cid) {
    ...
    if (.consecutive_api_blocks_tox >= 10L) {
        return(data.frame(
    ...
    is_network_err <- inherits(resp, "error") || (http_error(resp) && status_code(resp) %in% c(429, 502, 503, 504, 404))
    if (is_network_err) {
        .consecutive_api_blocks_tox <<- .consecutive_api_blocks_tox + 1L
    } else {
        .consecutive_api_blocks_tox <<- 0L
    }
