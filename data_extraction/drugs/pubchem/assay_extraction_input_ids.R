...
if (length(args) >= 2 && args[1] == "--batch_input_ids") {
    batch_ids_path <- args[2]
}
if (is.null(batch_ids_path) || !nzchar(batch_ids_path)) {
    stop("Usage: Rscript assay_extraction_input_ids.R --batch_input_ids <batch_input_ids.csv>")
}
...
u_expanded <- u[, .(input_id = unlist(strsplit(mapped_name, ";\\s*"))), by = cid]
...
for (b in seq_along(batches)) {
    ...
    for (aid in sub_aids) {
        ...
        Sys.sleep(0.15) # throttle a bit; tune as needed
    }
